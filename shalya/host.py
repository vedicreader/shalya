...
# %% auto #0
__all__ = ['SANDBOX', 'SECRET', 'NO_ROOTS', 'DENY', 'SKIP_DIRS', 'SKIP_SUFFIXES', 'MAX_VARS', 'LD_CHARS', 'MAX_PAGE_CHARS',
           'THIN_PAGE', 'CONTENT_SELECTORS', 'BLOCK_SELECTOR', 'READERS', 'Capability', 'Host', 'CodeHost', 'WebHost',
           'NotebookHost', 'MemoryHost', 'AskHost', 'WatchHost', 'SessionHost', 'ShellHost', 'ApiHost', 'GitHost',
           'denied', 'ld_json', 'LocalHost', 'md_title', 'read_page', 'implemented']
...
BLOCK_SELECTOR = ('article h1, article h2, article h3, article h4, article p, article pre, '
    'article blockquote, article li, article table, main h1, main h2, main h3, main h4, main p, '
    'main pre, main blockquote, main li, main table, [role="main"] h1, [role="main"] h2, '
    '[role="main"] h3, [role="main"] p, [role="main"] pre, [role="main"] blockquote, '
    '[role="main"] li, [role="main"] table')
...
def _quality(text):
    "Prose over a list of links: a page of bullets is navigation, not the article."
    lines = [x for x in text.splitlines() if x.strip()]
    return len(text) * (1 - sum(x.lstrip().startswith(('* ', '- ')) for x in lines) / max(1, len(lines)))

def _sections(f, page, mx):
    "Repeated `<article>` elements, each rendered on its own. A feed is many documents, not one."
    try: elements = list(page.css('article'))
    except Exception: return []
    out, seen = [], set()
    for el in elements:
        try:
            head = el.css('h1, h2, h3').first
            title = head.get_all_text().strip() if head is not None else ''
            text = str(f.to_md(str(el.get())) or '').strip()
            if len(list(el.css('article'))) > 1:
                title = next((l.strip().lstrip('#').strip()[:120] for l in text.splitlines() if l.strip()), '')
        except Exception: continue
        key = ' '.join(text.lower().split())
        if len(text) < 80 or key in seen: continue
        seen.add(key)
        out.append({'title': title or md_title(text, ''), 'text': text[:mx]})
    return out if len(out) > 1 else []

def _page_text(f, page, sel, mx):
    "The best markdown a page yields, which selector produced it, and its repeated articles."
    got, sections = [], (_sections(f, page, mx) if not sel else [])
    def take(label, **kw):
        try: text = str(f.to_md(page, **kw) or '').strip()
        except Exception: return
        if text: got.append((label, text))
    if sel: take(f'selector {sel}', sel=sel, multi=True)
    take('readability')
    for s in CONTENT_SELECTORS: take(s, sel=s, multi=True)
    take('semantic blocks', sel=BLOCK_SELECTOR, multi=True)
    if not got: return '', 'no extraction matched', sections
...
READERS = (
    ...
    ('youtube.com/', '', 'read_yt', 'page', ('source', 'transcript')),
    ('youtu.be/', '', 'read_yt', 'page', ('source', 'transcript')),
)

def _reader(f, url, say):
    "What a dedicated reader made of `url`, as `(text, title, kind)`. All three empty where none fits."
    for host_part, also, name, kind, keys in READERS:
        if host_part not in url or (also and also not in url): continue
        if (fn := getattr(f, name, None)) is None: continue
        kw = dict(save_pdf=False) if name == 'read_arxiv' else {}
        try: got = fn(url, **kw)
        except Exception as e:
            say(f'{name} failed ({host_err(e)})')   # a reader that gives up is not a page that cannot be read
            return '', '', ''
        if not keys: return str(got or ''), '', kind
        if not isinstance(got, dict): return str(got or ''), '', kind
        return str(next((got[k] for k in keys if got.get(k)), '')), str(got.get('title') or ''), kind
    return '', '', ''
...
def read_page(fossick, url, sel=None, note=None, mx=MAX_PAGE_CHARS):
    ...
    def _try(what, fn, *a, **kw):
        try: return fn(*a, **kw)
            ...
            say(f'{what} failed ({host_err(e)})')
            return None

    text, title, kind = _reader(f, url, say)
    if text.strip():
        return AttrDict(text=text[:mx], url=url, title=title or md_title(text, url), kind=kind,
                        strategy='', sections=[])

    page = _try('fetch page', f.fetch, url, auto=True)
    text, strategy, sections = ('', '', []) if page is None else _page_text(f, page, sel, mx)
    if len(text) < THIN_PAGE:
        for opts, how in (({'heavy': True, 'network_idle': True}, 'rendered'), ({'stealthy': True}, 'stealthy')):
            harder = _try(f'{how} page', f.fetch, url, **opts)
            if harder is None: continue
            richer, how_sel, more = _page_text(f, harder, sel, mx)
            if len(richer) > len(text): page, text, strategy = harder, richer, f'{how} {how_sel}'
            if len(more) > len(sections): sections = more
            if len(text) >= THIN_PAGE: break
    if len(text) < THIN_PAGE and hasattr(f, 'extract'):
        rows = _try('extract page', f.extract, url)
        rows = rows if isinstance(rows, list) else [rows]
        got = '\n\n'.join(x for x in (_body(r) for r in rows) if x)
        if len(got) > len(text): text, strategy = got[:mx], 'independent extraction'
    if not text.strip(): return None
    if (ld := ld_json(getattr(page, 'html_content', '') or '')):
        text = f'<structured-data>\n{json.dumps(ld)[:LD_CHARS]}\n</structured-data>\n\n{text}'
    return AttrDict(text=text[:mx], url=url, title=md_title(text, url), kind='page',
                    strategy=strategy, sections=sections)
...
def read_url(self:LocalHost, url, remember=True):
    "One page as markdown. The readers, the escalation and the JSON-LD are `read_page`'s."
